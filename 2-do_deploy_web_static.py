...
def do_deploy(archive_path):
    ...
    if not local("test -f {}".format(archive_path)).succeeded:
        ...
        return False
    """Upload archive with more descriptive variable"""
    archive_name = os.path.basename(archive_path)
    for server in env.hosts:
        put(archive_path, f"/tmp/{archive_name}")

    """Extract and organize files on web servers"""
    for server in env.hosts:
        with sudo(server):
    ...
    """Nginx restart assumed in separate script (replace path)"""
    for server in env.hosts:
        run("/path/to/restart_nginx.sh")
        return True

"""Example usage (assuming you have SSH key configured and username set as 'ubuntu')"""
env.user = "ubuntu"
env.hosts = ["3.86.7.100", "100.26.212.112"]

"""Get archive path from do_pack (replace with your implementation)"""
